...
const COMPANY_STATUS_CONFIG: Record<string, { label: string; badge: string; dot: string }> = {
  ACTIVE:    { label: "Ativa",    badge: "bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800", dot: "bg-emerald-500" },
  TRIAL:     { label: "Trial",    badge: "bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-800",          dot: "bg-blue-500" },
  SUSPENDED: { label: "Suspensa", badge: "bg-red-50 text-red-700 border-red-200 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800",             dot: "bg-red-500" },
  INACTIVE:  { label: "Inativa",  badge: "bg-zinc-100 text-zinc-600 border-zinc-200 dark:bg-zinc-800 dark:text-zinc-400 dark:border-zinc-700",      dot: "bg-zinc-400" },
};
...
function StatCard({
  ...
  return (
    <div
      onClick={onClick}
      className={cn(
        "bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5 flex items-start gap-4",
...
function HBar({ label, value, max, color }: { label: string; value: number; max: number; color: string }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-xs text-zinc-500 w-24 text-right shrink-0">{label}</span>
      <div className="flex-1 h-2 bg-zinc-100 dark:bg-zinc-800 rounded-full overflow-hidden">
        <div className={cn("h-full rounded-full transition-all duration-500", color)} style={{ width: `${pct}%` }} />
      </div>
      <span className="text-xs font-medium text-zinc-700 dark:text-zinc-300 w-8 text-right shrink-0">{value}</span>
    </div>
  );
}
  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
      ...
      <div
        ...
        className="w-full flex items-center justify-between px-5 py-4 border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer"
      >
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-zinc-400 truncate">
                    {s.equipment.name}
                    {s.client && <> · <span className="text-zinc-500">{s.client.name}</span></>}
  ...
  return (
    <div className="space-y-6 animate-pulse">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5 h-24" />
        ))}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {[1, 2, 3].map((i) => (
          <div key={i} className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6 h-52" />
        ))}
      </div>
    </div>
  );
  ...
  if (isError || !data) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        ...
        <p className="text-sm text-zinc-500">Não foi possível carregar o dashboard.</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Ordens de serviço</h2>
          </div>
          <div className="flex items-baseline gap-2 mb-4">
            <span className="text-3xl font-bold text-zinc-900 dark:text-zinc-100">{osMetrics.total}</span>
            <span className="text-sm text-zinc-400">total na plataforma</span>
          </div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Alertas</h2>
            </div>
            {totalAlerts > 0 && (
          ...
          {totalAlerts === 0 ? (
            ...
            <div className="space-y-2">
              ...
              <AlertRow
                ...
                icon={XCircle}
                color="bg-zinc-50 text-zinc-600 border-zinc-200 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-400"
                onClick={() => router.push("/empresas")}
              />
              <AlertRow
              ...
              <AlertRow
                ...
                icon={Users}
                color="bg-zinc-50 text-zinc-500 border-zinc-200 dark:bg-zinc-800 dark:border-zinc-700"
              />
            </div>
          )}
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Empresas por status</h2>
          </div>
          <div className="space-y-3">
            ...
            <HBar label="Inativas" value={companyMetrics.byStatus.inactive} max={Math.max(companyMetrics.total, 1)} color="bg-zinc-300" />
          </div>
          <div className="mt-4 pt-3 border-t border-zinc-100 dark:border-zinc-800">
            <p className="text-xs text-zinc-400">
              Total:{" "}
              <span className="font-semibold text-zinc-700 dark:text-zinc-300">{companyMetrics.total}</span>{" "}
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
          ...
          {licenseAlerts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-center px-4">
              <CheckCircle2 className="w-8 h-8 text-emerald-400 mb-2" />
              <p className="text-sm text-zinc-400">Nenhuma licença vencendo nos próximos 30 dias</p>
            </div>
          ) : (
            <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
              {licenseAlerts.map((company) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100 truncate">{company.name}</p>
                      <p className="text-xs text-zinc-400">{company._count.users} usuário{company._count.users !== 1 ? "s" : ""}</p>
                    </div>
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100 dark:border-zinc-800">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Cadastros recentes</h2>
            </div>
            <Button variant="ghost" size="sm" className="h-7 text-xs" onClick={() => router.push("/empresas")}>
              Ver todas <ArrowRight className="w-3.5 h-3.5 ml-1.5" />
            </Button>
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <span className="text-xs text-zinc-400">{company._count.users} usuário{company._count.users !== 1 ? "s" : ""}</span>
                        <span className="text-zinc-300">·</span>
                        <span className="text-xs text-zinc-400">{company._count.clients} cliente{company._count.clients !== 1 ? "s" : ""}</span>
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <Badge variant="outline" className={cn("text-xs block mb-1", statusCfg.badge)}>
                      ...
                      <p className="text-xs text-zinc-400">{timeAgo(company.createdAt)}</p>
                    </div>
                  </div>
                );
              })}
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          icon={AlertTriangle}
          accent={osMetrics.urgent > 0 ? "bg-red-500" : "bg-zinc-400"}
        />
        <StatCard
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5">
          ...
          {alerts.total === 0 && alerts.equipmentUnderMaintenance === 0 && alerts.warrantyExpiring === 0 && upcomingSchedules.length === 0 ? (
            ...
            <div className="space-y-2">
              <AlertRow count={alerts.unassignedOs} label="OS sem técnico no painel" icon={ClipboardList} color="bg-red-50 text-red-700 border-red-200 dark:bg-red-900/20 dark:border-red-800 dark:text-red-400" />
              <AlertRow count={alerts.overdueAlerts} label="OS com alerta de atraso" icon={Clock} color="bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-900/20 dark:border-orange-800 dark:text-orange-400" />
              <AlertRow count={alerts.equipmentUnderMaintenance} label="equipamentos em manutenção" icon={Wrench} color="bg-amber-50 text-amber-700 border-amber-200 dark:bg-amber-900/20 dark:border-amber-800 dark:text-amber-400" />
              <AlertRow count={alerts.warrantyExpiring} label="garantias vencendo em 30 dias" icon={CalendarClock} color="bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800 dark:text-blue-400" />
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Equipamentos</h2>
          </div>
          <div className="flex items-baseline gap-2 mb-4">
            <span className="text-3xl font-bold text-zinc-900 dark:text-zinc-100">{equipmentMetrics.total}</span>
            <span className="text-sm text-zinc-400">cadastrados</span>
          </div>
          <div className="space-y-3">
            ...
            <HBar label="Inativos" value={equipmentMetrics.byStatus.inactive} max={Math.max(equipmentMetrics.total, 1)} color="bg-zinc-300" />
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Grupos de manutenção</h2>
          </div>
          {groupMetrics.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              <Users className="w-8 h-8 text-zinc-300 mb-2" />
              <p className="text-sm text-zinc-400">Nenhum grupo ativo</p>
            </div>
          ) : (
            <div className="space-y-2.5">
              {groupMetrics.slice(0, 5).map((g) => (
                <div key={g.group_id} className="flex items-center gap-2.5">
                  ...
                  <span className="text-xs text-zinc-600 dark:text-zinc-400 flex-1 truncate">{g.group_name}</span>
                  <div className="flex items-center gap-1 text-xs">
                    <span className="font-semibold text-zinc-900 dark:text-zinc-100">{g.total_os}</span>
      ...
      {equipmentMetrics.byType && equipmentMetrics.byType.length > 0 && (
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
          <div
            ...
            className="w-full flex items-center justify-between gap-4 px-5 py-4 hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer"
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Equipamentos por tipo</p>
                ...
                return (
                  <div key={item.id}>
                    <div className="flex items-center justify-between mb-1.5">
                      <span className="text-sm text-zinc-700 dark:text-zinc-300 font-medium">{item.name}</span>
                      <span className="text-sm text-zinc-500 tabular-nums">
                        {item.count} <span className="text-xs text-zinc-400">({pct}%)</span>
                      </span>
                    </div>
      ...
      {topTechnicians.length > 0 && (
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
          <div className="px-5 py-4 border-b border-zinc-100 dark:border-zinc-800 flex items-center gap-2">
            <TrendingUp className="w-4 h-4 text-zinc-400" />
            <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">Top técnicos — últimos 30 dias</h2>
          </div>
          <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
              ...
              return (
                <div key={t.technician_id} className="flex items-center gap-4 px-5 py-3">
                  ...
                  <div className="flex-shrink-0 text-right">
                    ...
                    <p className="text-xs text-zinc-400 mt-1">{rate}%</p>
                  </div>
                </div>
              );
            })}
...
const OS_STATUS_LABELS: Record<string, { label: string; color: string }> = {
  AWAITING_PICKUP: { label: "No Painel",        color: "bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400" },
  OPEN:            { label: "Atribuída",         color: "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400" },
  IN_PROGRESS:     { label: "Em Andamento",      color: "bg-violet-100 text-violet-700 dark:bg-violet-900/40 dark:text-violet-400" },
  COMPLETED:       { label: "Aguard. Aprovação", color: "bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400" },
  COMPLETED_APPROVED: { label: "Aprovada",       color: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400" },
  COMPLETED_REJECTED: { label: "Reprovada",      color: "bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400" },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 p-5">
          <div className="flex items-center gap-2 mb-4">
            <ClipboardList className="w-4 h-4 text-zinc-400" />
            <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">OS por status</h2>
          </div>
          <div className="space-y-2.5">
            <HBar label="No Painel"    value={osMetrics.byStatus.awaitingPickup} max={Math.max(osMetrics.total, 1)} color="bg-zinc-400" />
          ...
          {osMetrics.avgResolutionHours != null && (
            <div className="mt-4 flex items-center gap-2 p-2.5 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800">
      ...
      {recentOs.length > 0 && (
        <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
          <div className="px-5 py-4 border-b border-zinc-100 dark:border-zinc-800 flex items-center gap-2">
            <ClipboardList className="w-4 h-4 text-zinc-400" />
            <h2 className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">OS recentes</h2>
          </div>
          <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
            {recentOs.map((os) => {
              const statusCfg = OS_STATUS_LABELS[os.status] ?? { label: os.status, color: "bg-zinc-100 text-zinc-600" };
              const priorityCfg = PRIORITY_LABELS[os.priority] ?? { label: os.priority, dot: "bg-zinc-400" };
              const technician = os.technicians[0]?.technician;
              return (
                <div key={os.id} className="flex items-center gap-4 px-5 py-3">
                  <span className="text-xs font-bold text-zinc-400 w-10 shrink-0">#{os.number}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100 truncate">{os.title}</p>
                    <p className="text-xs text-zinc-400 truncate">
