...
interface TecnicoStatsBarProps {
  orders: ServiceOrder[]
  isLoading: boolean
}

export function TecnicoStatsBar({ orders, isLoading }: TecnicoStatsBarProps) {
  const stats = [
    {
      label: 'No Painel',
    ...
    },
    {
      ...
      value: orders.filter(
        (o) =>
          o.priority === 'URGENT' &&
          !['COMPLETED_APPROVED', 'CANCELLED', 'COMPLETED_REJECTED'].includes(o.status),
      ).length,
      color: 'text-red-700',
      dot: 'bg-red-500',
    },
  ...
  if (isLoading) {
    return (
      <div className="flex items-center gap-1 px-4 py-2 bg-white dark:bg-zinc-950 border-b border-[#e0e5eb] dark:border-zinc-800 ">
