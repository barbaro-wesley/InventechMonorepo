...
export class LaudoTemplatesController {
  ...
  clone(@Param('id') id: string, @Req() req: any) {
    return this.service.clone(id, req.user.companyId, req.user.sub, req.user.clientId ?? null)
  }
  ...
  create(@Body() dto: CreateLaudoTemplateDto, @Req() req: any) {
    ...
    return this.service.create({ ...dto, fields }, req.user.companyId, req.user.sub, req.user.clientId ?? null)
  }

  @Get()
  @Permission('laudo-template:list')
  findAll(@Query() filters: ListLaudoTemplatesDto, @Req() req: any) {
    return this.service.findAll(req.user.companyId, filters, req.user.clientId ?? null)
  }
  ...
  update(@Param('id') id: string, @Body() dto: UpdateLaudoTemplateDto, @Req() req: any) {
    ...
    return this.service.update(id, safeDto, req.user.companyId, req.user.clientId ?? null)
  }

  @Delete(':id')
  @Permission('laudo-template:delete')
  remove(@Param('id') id: string, @Req() req: any) {
    return this.service.remove(id, req.user.companyId, req.user.clientId ?? null)
  }
}
