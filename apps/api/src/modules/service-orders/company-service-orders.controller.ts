...
export class CompanyServiceOrdersController {
    ...
    createChild(
        ...
        return this.serviceOrdersService.createChild(id, dto, cu.clientId ?? null, cu.companyId!, cu)
    }
    ...
    async updateStatus(
        ...
        const os = await this.serviceOrdersService.updateStatus(id, dto, null, cu.companyId!, cu)
        if (files?.length > 0) {
            await Promise.all(
                files.map((file) =>
                    this.storageService.upload(
